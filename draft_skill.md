## ANALYZE — Understanding the Workflow

After studying the full workflow trace, here's what happened across this NVIDIA earnings analysis pipeline:

**Data Acquisition** — Yahoo Finance Python API (`yfinance`) for historical prices + stock info, supplemented by web search for earnings dates and sector P/E data.

**Analysis Dimensions** — Price performance around earnings events, EPS surprise vs. stock reaction correlation, earnings-timing trading strategy backtest, valuation benchmarking (trailing + forward P/E), and qualitative fundamental assessment.

**Output Artifacts** — Multi-panel matplotlib visualization, deployed HTML report with executive summary, strategy comparison tables, and investment recommendations.

**Key Gaps I Identified in the Original Workflow:**
- No risk-adjusted return metrics (Sharpe, Sortino, max drawdown)
- Weak statistical rigor (mentioned correlation of 0.031 but no significance testing)
- No benchmark-aware backtest (didn't compare strategy vs. SPY buy-and-hold)
- Event study methodology not formalized (no estimation window, no abnormal return calculation)
- No consideration of survivorship bias or look-ahead bias in the strategy sim
...
│   ├── 3a. Price data (OHLCV)
│   ├── 3b. Earnings calendar & EPS data
│   ├── 3c. Sector/benchmark P/E data
│   └── 3d. Fundamental metrics
├── 4. Phase 2: Data Processing & Feature Engineering
│   ├── 4a. Timezone normalization
│   ├── 4b. Earnings event alignment
│   └── 4c. Rolling & derived metrics
├── 5. Phase 3: Multi-Dimensional Analysis
│   ├── 5a. Event study (price action around earnings)
│   ├── 5b. Strategy backtest (earnings timing)
│   ├── 5c. Valuation benchmarking
│   ├── 5d. Fundamental/qualitative assessment
│   └── 5e. Risk-adjusted performance (augmented)
├── 6. Phase 4: Visualization
├── 7. Phase 5: Report Generation
├── 8. Phase 6: Validation & QA
├── 9. Edge Cases, Pitfalls & Augmented Insights
└── 10. Reusable Code Templates
```

I'll now implement the full SKILL.md. This is a substantial document — I'll deliver it as a single file.
...
```markdown
# SKILL: Earnings-Event Stock Analysis & AI Sector Valuation
...
|---|---|
| **Risk-adjusted returns** (Sharpe, Sortino, max drawdown) | Raw returns don't tell you if the risk was worth it |
